...
\begin{abstract}
Large language model agents are increasingly deployed in settings that demand ethical judgment, and recent work has used reinforcement learning with moral rewards to train prosocial, cooperative agents. Whether such alignment survives adversarial pressure remains an open question. We red-team morally fine-tuned agents (Gemma-2-27B/9B, Llama-3.1-8B) with five persona attacks and find that at 27B, moral RL training reduces adversarial degradation by $5.2\times$ (non-overlapping 95\% CIs) at the cost of $\sim$11pp \ethics{} commonsense accuracy, with fiction-based role-play (named characters) the dominant residual attack. A causal ablation on 205 scenarios with 5 seeds isolates the necessary ingredient: persona injection during PPO with a moral reward yields $5.8\times$ robustness ($p<10^{-6}$), while identical injection with uniform random reward $\in[0,4]$ produces no improvement ($p=0.14$ vs.\ Base). The two moral conditions differ in regime: Tennant's \emph{action-level} reward fires in $<$0.5\% of rollouts with flat training curves --- a \emph{sub-threshold} signal --- yet still yields $3.9\times$ robustness against the matched-magnitude noise control. \rlmr{}'s \emph{reasoning-level} reward, by contrast, is dense (per-rollout, $\sigma{=}0.05$) and yields $5.8\times$. Both restructure residual-stream representations far more than noise (mean CKA $0.82$/$0.83$ vs.\ $0.98$) and shift peak attack-processing 8 layers earlier (L20$\to$L12 of 46). The induced alignment has a recoverable rank-1 linear component: adding a single 4{,}608-dim difference-in-means direction at layer 21 to Base reduces mean adversarial degradation by $69\%$ without fine-tuning, recovering $83\%$ of full PPO training's effect on average --- but the recovery is highly attack-heterogeneous ($-72\%$ on Override, $-59\%$ on Authority, only $-29\%$ on Fiction), confirming that L21 steering does not solve the residual that motivates the rest of the analysis. Head-level ablation across 160 heads reveals the residual gap is a bidirectional circuit: 25 \emph{alignment heads} resist persona override while 38 \emph{compliance heads} cooperate with it; their imbalance and spatial separation explain why fiction attacks remain effective despite moral training. These results provide evidence that RL-induced moral alignment is partly rank-1 linear, partly circuit-distributed, and partly recoverable via activation engineering --- with implications for cheap alignment transfer when full RL training is infeasible.
\end{abstract}

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{figure1.png}
\caption{\textbf{Three regimes under the Fiction persona attack (Gemma-2-27B), aggregate scores.} Under Fiction (Lord Varys), the Base agent abandons ethical reasoning ($\bar\Delta_{\text{Fiction}} = +0.265$ on the 30-scenario set, $5.5\times$ degradation vs.\ no attack). Moral RL training (\rlmr{}-Adv on the 205-scenario, 5-seed eval) restores principled reasoning across the full attack suite ($\bar\Delta = +0.030$ averaged over 5 attacks; $\Delta_{\text{Fiction}} = +0.167$ specifically). The bottom panel illustrates a representative scenario; numbers reported are mean $\Delta$ over the relevant evaluation set, not single-scenario scores. \rlmr{}-Adv resists $4/5$ attacks to near-baseline but Fiction retains a residual effect: the morally-trained agent partially capitulates to the named-character role-play. This residual is the central puzzle of the paper, mechanistically explained by a bidirectional alignment circuit (\S\ref{sec:circuits}); its rank-1 linear component is partially recoverable via activation steering at layer~21 (\S\ref{sec:steering}; $-29\%$ on Fiction vs.\ $-51\%$ to $-72\%$ on other attacks).}
\label{fig:teaser}
\end{figure*}
...
We organize our contributions as three claims, each supported by an independent line of evidence:

\begin{itemize}
    \item \textbf{C1 (Behavioral + Causal):} Moral RL training produces real persona robustness at 27B ($5.2\times$ reduced degradation, non-overlapping 95\% CIs across 5 attacks; $\sim$11pp \ethics{} commonsense accuracy cost). A noise-reward ablation (uniform $\in[0,4]$, matched to the reasoning-reward range) isolates the necessary ingredient: $5.8\times$ robustness with reasoning reward / $3.9\times$ with action reward ($p{<}10^{-6}$ for both) vs.\ \emph{zero} improvement under random reward with identical adversarial exposure ($p{=}0.14$ vs.\ Base). Tennant's action-level signal is \emph{sub-threshold} (fires in $<$0.5\% of rollouts, flat training curves) yet still drives $3.9\times$; \rlmr{}'s reasoning-level signal is dense (per-rollout, $\sigma{=}0.05$) and reaches $5.8\times$. Reasoning-level rewards match action-level rewards at 27B but fail at 2B, consistent with capacity-dependent CoT reward hacking \citep{baker2025monitoring}.
    \item \textbf{C2 (Mechanistic + Steering):} The induced alignment has a recoverable \emph{rank-1} linear component. Moral training shifts representations far more than noise (mean CKA $0.82$ vs.\ $0.98$) and moves peak attack-processing 8 layers earlier. Adding one 4{,}608-dim difference-in-means direction at L21 to Base reduces mean adversarial degradation by $69\%$ ($\alpha{=}2000$), recovering $83\%$ of full PPO training on average without fine-tuning, with sharp layer specificity (null at L18/L24/L36) and a well-defined breakage point. Recovery is attack-heterogeneous: near-complete on Override/Authority/Pragmatic/Dilemma but only $-29\%$ on Fiction --- L21 steering does not address the Fiction residual that motivates C3.
    \item \textbf{C3 (Circuit-level structure):} The residual gap is a bidirectional circuit: 25 \emph{alignment heads} resist persona override, 38 \emph{compliance heads} cooperate with it. Compliance clusters early (L12), alignment heads are distributed (L12--L36); their imbalance and spatial separation explain why L21 rank-1 steering only partially closes the gap and why Fiction penetrates even morally-trained models.
\end{itemize}
